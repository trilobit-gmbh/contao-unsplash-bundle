...
class UnsplashApi
{
    ...
    {
        //$strApiUrl = Config::get('unsplashApiUrl');
        $strApiUrl = Helper::getConfigData()['api_url'];
        $strApiKey = Config::get('unsplashApiKey');
        ...
        if ('all' === $arrApiParameter['orientation']) {
            unset($arrApiParameter['orientation']);
        }
        ...
        $strCacheFile = $strCachePath.'/contao/unsplash/'.$strChecksum.'.json';
        ...
        if (null === $arrResult) {
            try {
                ...
                System::log('Unsplash search failed: '.$e->getMessage(), __METHOD__, TL_ERROR);
                $arrResult = [];
    ...
    {
        // check query
        if (!Input::get('query') || '' === Input::get('query')) {
            self::apiResponse();
        }

        $strResponseGroup = 'download_location';
    ...
    {
        $strUrl = $strApiUrl.'search/photos/?client_id='.$strApiKey.'&'.http_build_query($arrApiParameter);

        $objCurl = curl_init();

        curl_setopt($objCurl, CURLOPT_URL, $strUrl);

        curl_setopt($objCurl, CURLOPT_USERAGENT, 'Contao Unsplash API');
